...
@app.get("/beat-success-rates")
def get_beat_success_rates(
    district: str = Query(..., description="District name"),
    db: sqlite3.Connection = Depends(get_db),
):
    """Get beats with highest success rates (lowest crime counts) for a district."""
    cursor = db.cursor()
    cursor.execute(
        """
        SELECT Beat_Name, COUNT(*) as crime_count
        FROM Hack_DS
        WHERE District_Name = ?
        GROUP BY Beat_Name
    ...
    if not rows:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No beat data found for district: {district}",
        )

    # Prepare data for charting
    beats = [row["Beat_Name"] for row in rows]
    crime_counts = [row["crime_count"] for row in rows]
    ...
    return {
        ...
        "chart_data": {
            ...
            "x": beats,
            "y": crime_counts,
            "title": f"Beats with Highest Success Rates in {district} (Lowest Crime Counts)"
        }
    }
